...
def make_pairs(data_dir, split='train', setting='egoexo'):
    ...
    pairs = []
    not_found_count = 0  # Add counter for FileNotFoundError
    ...
    for take in split_takes:
        ...
        try:
            ...
            not_found_count += 1  # Increment counter
            print(f'{take} - FileNotFoundError')
            continue

    print(f'{split} - pairs: ', len(pairs))
    print(f'{split} - FileNotFoundError count: {not_found_count}')  # Print the count
